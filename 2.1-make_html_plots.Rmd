...
# all the plotting functions plot data: "long_processed_minimal" by default
  ...
  concWW_plot <- plot_mean_sd_jitter(measure_var = 'Copies_Per_Liter_WW', WWTP_fltr_var = wwtp_manhole_names, x_var = WWTP, ylabel = 'Genome copies/l Wastewater') 
...
plot_mean_sd_jitter(measure_var = 'Copies_Per_Liter_WW', target_filter_var = 'BCoV|BRSV', x_var = WWTP, ylabel = 'Genome copies/l Wastewater') %>%  format_logscale_y() %>% print()
...
```{r percentreco}
  ...
  percentreco_plt <- plot_mean_sd_jitter(measure_var = 'Percentage_recovery_BCoV', target_filter_var = 'BCoV|BRSV', x_var = WWTP, ylabel = 'Percentage recovery of surrogate virus') 
...
plot_mean_sd_jitter(WWTP_fltr_var = wwtp_manhole_names, exclude_WWTP = T, x_var = WWTP) %>% format_logscale_y() %>% print()


# Copies/l WW: N1/N2 and BCoV plots
plot_mean_sd_jitter(measure_var = 'Copies_Per_Liter_WW', WWTP_fltr_var = wwtp_manhole_names, exclude_WWTP = T, x_var = WWTP, ylabel = 'Genome copies/l Wastewater') %>% 
  format_logscale_y() %>% print()

# # recovered vs spiked in plot for extra samples
# plot_mean_sd_jitter(long_vol_qpcr_summary, long_vol_qpcr_raw, long_format = T, measure_var = 'Recovered', Sample_fltr_var = extra_samples_to_plot, exclude_sample = F, x_var = WWTP, ylabel = 'Genome copies/l Wastewater') %>%
#   format_logscale_y() %>% print()

# % plot for extra samples
plot_mean_sd_jitter(measure_var = 'Percentage_recovery_BCoV', WWTP_fltr_var = wwtp_manhole_names, exclude_WWTP = T, target_filter_var= 'BCoV|BRSV', x_var = WWTP, ylabel = 'Percentage recovery of surrogate virus') %>% print()
...
```{r percentRecoverystreamlined}
  ...
  recostr_plot <- plot_mean_sd_jitter(x_var = Percentage_recovery_BCoV, ascending_order = T, ylabel = 'Percentage recovery of surrogate')
