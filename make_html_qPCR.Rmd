...
# copies/ul viral suspension spiked in                                          (Spike ID: S19)
all_spike_virus_conc <- tibble(Target = c('BCoV_M', 'BRSV_N'), spike_virus_conc = c(7913.4, NA))
...
read_these_sheets <- c('WW35_716_Direct extr_BCoV_Std18', 'dd.WW10_716_David_Extracts')
title_name <- '716 Direct extr'

# Biobot ID sheet
bb_sheets <- c('Week 14 (7/13)')
...
# baylor's volumes from a separate excel file    
if (bb_qpcr$Target %>% str_detect('Baylor') %>% sum()) {
  baylor_volumes <- str_c('excel files/Baylor/', 'Baylor_sample volumes', '.xlsx') %>% 
    read_xlsx (sheet = '706') %>% 
    rename('WWTP' = contains('SYMBOL', ignore.case = T), 'FACILITY NAME' = matches('FACILITY NAME', ignore.case = T), 'Label_tube' = `Sample ID`) %>% 
    mutate_at('Label_tube', ~str_remove(., '\\.') ) %>% 
    ...
    select(-`FACILITY NAME`, -biological_replicates, -WWTP)
  }

# join the results with the volumes
vol_R <- bb_qpcr %>% 
  filter(!str_detect(Target, 'Baylor')) %>% 
  left_join(volumes_data, by = 'Label_tube') %>% 
...
vol_B <- bb_qpcr %>% 
  ...
  mutate(vol_extracted = 1.5) %>% 
  mutate(spike_virus_conc = all_spike_virus_conc %>% filter(str_detect(Target, 'BCoV') ) %>% pull(spike_virus_conc))  # spike in concentrations (BCoV and BRSV)
...
if(present_WW_data %>% is_empty() %>% !.){
  write_sheet(present_WW_data,'https://docs.google.com/spreadsheets/d/1dBESjgWSFsOBodFFpYNhWIOAQ2a3wvoLkrn12V_rFck/edit#gid=0', sheet = title_name) # save results to a google sheet
}
```
