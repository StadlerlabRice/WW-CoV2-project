...
all_spike_virus_conc <- tibble(Target = c('BCoV_M', 'BRSV_N'), spike_virus_conc = c(NA, NA))
...
if (bb_qpcr$Target %>% str_detect('Baylor') %>% sum()) {
  baylor_volumes <- str_c('excel files/Baylor/', 'Baylor_sample volumes', '.xlsx') %>% 
    read_xlsx (sheet = '706') %>% 
    rename('WWTP' = contains('SYMBOL', ignore.case = T), 'FACILITY NAME' = matches('FACILITY NAME', ignore.case = T)) %>% 
    mutate_at('Sample ID', ~str_remove(., '\\.') ) %>% 
    
    pivot_longer(cols = starts_with('Volume'), names_to = 'index', values_to = 'WW_vol') %>% 
    separate(index, into =  c(NA, 'biological_replicates')) %>% 
    select(-`FACILITY NAME`) %>% 
    mutate(`Sample ID` = str_c(`Sample ID`, '.', biological_replicates), WW_tr = WW_vol %>% setNames(`Sample ID`))
  }
...
vol_qpcr <- bb_qpcr %>% left_join(volumes_data, by = 'Label_tube') %>% 
  ...
  {if (str_detect(.$Target, 'Baylor') %>% sum()) mutate_cond(., str_detect(.$Target, 'Baylor'),  'WW_vol' = str_replace_all(WW_vol, baylor_volumes$WW_tr)) } %>% 
