...
results_abs %>% ggplot(aes(x = Week, y = `Recovery fraction`)) + geom_jitter(width = .1) + geom_violin()
```
...
results_abs %>% filter(str_detect(Target, 'N1|N2')) %>% 
  ggplot(aes(x = Week, y = `Copies/l WW`, colour = Target)) + geom_jitter(width = .1) + geom_violin()
```
...
results_abs.list <- list(
  ...
  summ.dat = results_abs %>% select(Facility, WWTP, Target, `Copies/ul RNA`: `Recovery fraction`, Week, -any_of('Bottle')) %>%
...
summary_reco <- results_abs %>% select(Facility, WWTP, Target, `Copies/ul RNA`: `Recovery fraction`, Week, -any_of('Bottle')) %>%
  mutate('Sample_name' = Week) %>% 
...
plt_week3 <- plot_mean_sd_jitter(recovery_data.list, long_format = F, sample_var = extra_categories, exclude_sample = T, x_var = Week, y_var = `Recovery fraction`, colour_var = Target, ylabel = 'Percentage of BCoV copies recovered') + geom_line(aes(group = Target)) + facet_wrap(facets = ~WWTP, scales = 'free_x')
...
summary_normalized <- results_abs %>% select(Facility, WWTP, Target, `Copies/ul RNA`: `Recovery fraction`, Week, `Viral load per capita per day`, -any_of('Bottle')) %>%
  mutate('Sample_name' = Week) %>% 
  group_by(`Sample_name`, Target, Facility, WWTP, Week) %>% 
