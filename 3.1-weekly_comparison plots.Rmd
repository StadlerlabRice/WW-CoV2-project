...
<!-- plt_week4 <- plot_mean_sd_jitter(normalized_copies_N12, long_format = F, sample_var = extra_categories, exclude_sample = T, x_var = Week, y_var = `Viral load per capita per day`, facet_var = WWTP, colour_var = Target_Name, ylabel = 'Viral load per capita per day', facet_style = 'manual') + geom_line(aes(group = Target_Name, y = mean_Copies_Per_Liter_WW)) + facet_wrap(facets = ~WWTP, scales = 'free_x') -->
