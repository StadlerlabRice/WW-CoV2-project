...
for (wwtp in unique(results_abs$WWTP)) {
  ...
  if (wwtp %in% c("BRIE", "PARE")) {
    p <- p + geom_vline(xintercept = as.numeric(results_abs$Date[100523]), linetype = "dashed", color = "red")
  } else if (wwtp == "SCAE") {
    p <- p + geom_vline(xintercept = as.numeric(results_abs$Date[102423]), linetype = "dashed", color = "blue")
  }
