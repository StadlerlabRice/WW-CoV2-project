...
BCoV_data_to_plot <- bcov_data %>% 
  filter(!str_detect(WWTP, 'PT|NE')) %>% 
  mutate(across(Facility, ~ if_else(is.na(.x), 'Water', Facility))) %>% 
  mutate(Target = 'BCoV_M') # dummy column, only necessary for plotting
...
scatter_N_recovery <- N_data_to_plot %>% 
...
aligned_shape_plt_N2 <- {N_data_to_plot %>% 
...
aligned_shape_plt_recovery <- {BCoV_data_to_plot %>% 
