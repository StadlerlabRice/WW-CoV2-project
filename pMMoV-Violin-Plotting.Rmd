...
y_var = Copies_Per_Liter_WW, colour_var = Run_week, ylabel = 'Genome copies/l Wastewater', print_the_plot = FALSE) + 
  ...
  geom_vline(xintercept = as.numeric(as.Date("2023-10-05")), color = "red", linetype = "dashed", 
             data = results_abs[results_abs$WWTP %in% c("BRIE", "PARE"), ]) +
  geom_vline(xintercept = as.numeric(as.Date("2023-10-24")), color = "blue", linetype = "dashed", 
             data = results_abs[results_abs$WWTP == "SCAE", ])
} %>% 
  print()
# }
